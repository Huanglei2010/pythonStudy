...
def main():
    root=Tk()
    fm=Frame(root,width=300,height=400)
    fm.pack()
    random.seed(64)
    # hash(64)is used
    ...
    Label(fm,text="Start of evolution").pack()
    ...
    Label(fm,text="  Evaluated %i individuals" % len(pop)).pack()
    ...
    for g in range(NGEN):
        ...
        Label(fm,text="  Min %s" % min(fits)).pack()
        print("  Max %s" % max(fits))
        Label(fm,text="  Max %s" % max(fits)).pack()
        print("  Avg %s" % mean)
        Label(fm,text="  Avg %s" % mean).pack()
        print("  Std %s" % std)
        Label(fm,text="  Std %s" % std).pack()

    print("-- End of (successful) evolution --")
    Label(fm,text="-- End of (successful) evolution --").pack()
    best_ind = tools.selBest(pop, 1)[0]
    print("Best individual is %s, %s" % (best_ind, best_ind.fitness.values))
    Label(fm,text="Best individual is %s, %s" % (best_ind, best_ind.fitness.values)).pack()
    root.mainloop()

if __name__ == "__main__":
    main()

# 网址 https://segmentfault.com/a/1190000004345123